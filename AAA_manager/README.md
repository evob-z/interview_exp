...
| `main.py` | CLI 入口（extract / review / archive / sync / reflect / prepare / export-session） |
| `config.py` | 配置（从 `.env` 加载） |
| `llm_client.py` | LLM 调用封装（OpenAI SDK 兼容 DeepSeek / Qwen） |
| `extractor.py` `archiver.py` `reviewer.py` `reflector.py` `preparer.py` | 复盘流水线核心模块 |
| `detector.py` `git_ops.py` `logger.py` | 变更检测 / Git 操作 / 日志 |
| `api/routes/` | FastAPI 路由：qa / profile / history / followup / asr / sync / stats / prepare |
| `core/` | `web_searcher.py`（Tavily / Bing / Serper）、`asr_xunfei.py`（讯飞流式 ASR）、`prepare_agent.py`（岗位预测 ReAct Agent，基于 OpenAI function calling） |
...
| `prompts/` | LLM Prompt 模板（Markdown，含 reflect_agent_system / reflect_summary_system） |
...
# 或
python main.py sync          # 命令行全流程（自动检测新增文件）
python main.py sync --reflect  # 全流程 + 反思交互
```
