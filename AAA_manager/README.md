...
| 空对话自动清理 | 启动时自动删除无消息空会话（保护60秒内新建会话） | 自动 |
| 历史对话批量处理 | 删除/一条龙（导出→复盘→入库）历史会话 | Web 界面左侧边栏 |
| 会话导出 | 从 session 导出面试问题（支持 LLM 上下文感知改写） | CLI `export-session` |
| CLI 命令集 | extract / review / archive / sync / prepare / export-session | 命令行 |
...
```
AAA_manager/
├── main.py              # CLI 入口（extract/review/archive/sync/prepare）
...
```bash
conda create -n interview_assistant python=3.11 -y
...
| 区域 | 宽度 | 内容 |
|------|------|------|
| 左侧边栏 | 260px | 会话历史列表 + 新对话按钮 + 批量操作（删除/一条龙处理） |
...
- **空对话自动清理**：应用启动时自动删除无消息的空会话（保护 60 秒内新建的会话）
- **历史对话批量处理**：
  - 🗑️ 删除按钮：手动删除不需要的历史会话
  - ⚡ 一条龙处理：对历史会话执行导出→复盘→入库全流程
- **会话导出**：从 session 中导出面试问题（支持 LLM 上下文感知改写）
...
```bash
# 三个独立工具（可单独执行）
python main.py extract <file> [--type transcript|chat|structured]   # 抽取问题
python main.py extract --from-session <session_id>                  # 从模拟面试会话导出
python main.py review <file>       # 生成复盘分析（file 必须在 面试原始问题/ 目录）
python main.py archive <file>      # 归档入库（file 必须在 面试原始问题/ 目录）

# 全流程串联命令：extract → review → archive
python main.py sync <file> [--type transcript|chat|structured]

# 岗位针对性备战：搜 JD + LLM 出题，写入 岗位预测/【自动入题库】
...
`PROJECT_CONFIGS` 是**项目理解能力的入口**。注册后，`ProjectReader` 会对每个项目自动执行四层渐进式文档发现：

| 层级 | 内容 | 加载时机 |
|------|------|----------|
| Tier 1 | AI 工具总结（`.qoder/repowiki/`、`.cursor/rules/`、`.rules`） | 启动时 |
| Tier 2 | 设计文档（`README.md`、`ARCHITECTURE.md`、`docs/` 目录） | 启动时 |
| Tier 3 | 零散文档（根目录下其他 `.md` 文件） | 按需加载 |
| Tier 4 | 代码文件（`.py`、`.js` 等） | 锁定，需用户批准 |

**格式**：`项目名:盘符:/路径:向后兼容文件列表`，多个项目用 `;` 分隔

```env
PROJECT_CONFIGS=law_sea:D:/AI_model/law_sea:README.md;compliance_checker:D:/AI_model/compliance_checker:README.md
```

**注意**：
- 未在此注册的项目无法被 `archive` 入库时获取项目上下文
- 配置中的文件列表字段仅为向后兼容，实际 `discover()` 会自动扫描上述四层
- 新增项目后务必同步更新此配置
...
| `/api/sync/session-pipeline/{session_id}` | POST | 会话一条龙处理（导出→复盘→入库） |
...
| `/api/history/sessions/cleanup-empty` | DELETE | 清理空会话（启动时自动调用） |
...
覆盖模块：`config` / `detector`（Web API用） / `extractor` / `archiver` / `profile_manager` + 主要 API 路由。
